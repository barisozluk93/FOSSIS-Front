...
export class ChargingStationComponent implements OnInit, OnDestroy {

  @ViewChild('editSaveComponent') private editSaveComponent: ChargingStationEditSaveComponent;
  ...
  constructor(
    private umaterialManagementService: MaterialManagementService, 
  ...
  controlPermissions() {
    this.authService.currentUserSubject.asObservable().subscribe(result => {
      if(result?.permissions)
      {
        ...
        if(permissionList.includes(PermissionEnum['ChargingStationScene.Delete.Permission'])) {
        ...
        if(permissionList.includes(PermissionEnum['ChargingStationScene.Edit.Permission'])) {
        ...
        if(permissionList.includes(PermissionEnum['ChargingStationScene.Save.Permission'])) {
  ...
  delete(event: number) {
    this.umaterialManagementService.chargingStationDelete(event).subscribe(result => {
      if(result.isSuccess) {
